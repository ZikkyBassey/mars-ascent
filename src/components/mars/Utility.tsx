    ...
    <div className="container relative z-10">
      ...
      <div className="mt-20 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {items.map(({ icon: Icon, title, desc }, i) => (
          <div
            key={title}
            className="group glass-panel rounded-2xl p-7 relative overflow-hidden hover:-translate-y-2 transition-all duration-500 hover:border-mars-ember/60"
            style={{ transitionDelay: `${i * 50}ms` }}
          >
            <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                 style={{ background: "radial-gradient(circle at top, hsl(var(--mars-crimson) / 0.25), transparent 70%)" }} />
            <div className="relative">
              <div className="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-mars-crimson/15 border border-mars-crimson/40 text-mars-ember group-hover:shadow-[var(--glow-red)] transition-all duration-500">
