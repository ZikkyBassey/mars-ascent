...
const Roadmap = () => (
  <section id="roadmap" className="relative py-32 overflow-hidden">
    <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[800px] rounded-full bg-mars-crimson/10 blur-3xl pointer-events-none" />
    ...
    <div className="container relative z-10">
      ...
      <div className="mt-20 relative">
        ...
        <div className="space-y-10">
          {phases.map((p, i) => (
            <div key={p.tag} className={`md:grid md:grid-cols-2 md:gap-16 items-center ${i % 2 ? "" : "md:[&>*:first-child]:order-2"}`}>
              <div className={`glass-panel rounded-2xl p-7 relative ${i % 2 ? "md:text-right" : ""} hover:shadow-[var(--glow-red)] transition-all duration-500`}>
                <div className="absolute top-1/2 -translate-y-1/2 hidden md:block"
                     style={{ [i % 2 ? "left" : "right" as any]: "-3rem" }}>
                  <div className="h-4 w-4 rounded-full bg-mars-ember shadow-[0_0_20px_hsl(var(--mars-ember))]" />
                </div>
                ...
                <ul className={`mt-5 space-y-2 text-foreground/75 ${i % 2 ? "md:list-none" : ""}`}>
                  {p.items.map((it) => (
                    <li key={it} className="flex items-center gap-3 md:justify-start">
                      <span className="h-1 w-6 bg-mars-crimson" />
                      {it}
                    </li>
                  ))}
                </ul>
