...
const About = () => {
  ...
  return (
    <section id="about" className="relative py-20 md:py-32 overflow-hidden">
      ...
      <div className="container relative z-10 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center px-5 md:px-8">
        <div
          ref={left.ref}
        ...
        >
          ...
          <h2 className="mt-4 font-display text-4xl sm:text-5xl md:text-6xl font-black leading-[1.1]">
          ...
          <div className="glass-panel rounded-3xl p-8 md:p-10 relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-6 relative">
              {[
                ...
                <div
                  key={s.k}
                  className={`border border-mars-crimson/20 rounded-xl p-4 sm:p-5 bg-mars-black/40 hover:border-mars-ember/60 hover:-translate-y-1 hover:shadow-[var(--glow-red)] transition-all duration-500 ${right.visible ? "animate-fade-up" : "opacity-0"}`}
                  style={{ animationDelay: `${300 + i * 100}ms` }}
                >
                  <div className="text-[9px] sm:text-[10px] uppercase tracking-[0.25em] sm:tracking-[0.3em] text-mars-ember">{s.k}</div>
                  <div className="mt-2 font-display text-base sm:text-xl font-bold break-words">{s.v}</div>
                </div>
              ))}
            </div>
          </div>
